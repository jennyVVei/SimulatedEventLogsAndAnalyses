...
fun oas_by_type(ot: ObjectType) = 
	...
	if ot="Truck" then ["RFID No", "Pickup Plan ID", "Cargo ID", "LPT", "Axles", "Scheduled Pickup Weight", "Truck Status", "Truck Location Validity","Truck Weight", "Truck_Weight_Status"] else 
	if ot="Cargo" then ["Cargo Type", "Cargo stock weight(withholding)", "Cargo stock weight(real)", "Silo ID"] else 
